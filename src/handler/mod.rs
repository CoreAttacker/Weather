...
#[derive(Deserialize, Serialize, Debug)]
pub struct Coordinate {  //坐标
    lon: f64,
    lat: f64,
}

#[derive(Deserialize, Serialize, Debug)]
pub struct Sun {  //日出日落时间
    sunrise: i32,
    sunset: i32,
}

#[derive(Deserialize, Serialize, Debug)]
pub struct Weather {  //天气总数据
    main: Temperature,
    sys: Sun,
    coord: Coordinate,
}
...
impl Weather {
    ...
    let url = format!("http://api.openweathermap.org/data/2.5/weather?q={}&APPID=173e4eb815619f40c1a4fc3d952823e3", city);
   ...
   pub fn print_response(resp: &Weather) {
    ...
    resp.main.temp_min - 273.15,
    resp.main.temp_max - 273.15,
    resp.main.feels_like - 273.15,
    resp.main.humidity,
